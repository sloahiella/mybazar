...
function OrderReceipt({ order, onClose, isAdmin }: { order: any; onClose: () => void; isAdmin: boolean }) {
  ...
  function handleSave() {
    const lines = ['????? ????? | ??? ?????', 'sohelmart.com', 'WhatsApp: 01872149655', '----------------------------------------', `?????: ${new Date(order.created_at).toLocaleDateString('bn-BD')}`, `???: ${order.customer_name} | ???: ${order.customer_phone}`, `????: ${order.district}, ${order.upazila}`, `??????: ${order.address}`, `?????? #: ${order.id}`, '----------------------------------------', ...((order.order_items || []).map((item: any) => `${item.products?.name} × ${item.quantity} ${item.products?.unit} = ${(item.price * item.quantity).toFixed(0)} Tk`)), '----------------------------------------', `???????: ${order.total_amount} Tk`];
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '680px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderBottom: '1px solid #e5e7eb', position: 'sticky', top: 0, background: 'white', zIndex: 10 }}>
          <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: PINK, margin: 0 }}>?????? #{order.id}</h2>
          <div style={{ display: 'flex', gap: '8px' }}>
            {isAdmin && <button onClick={handlePrint} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '14px', cursor: 'pointer' }}>??? Print</button>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2px 1fr', border: `2px solid ${PINK}`, borderRadius: '8px', overflow: 'hidden', marginBottom: '16px' }}>
            <div style={{ padding: '14px' }}>
              ...
              <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>???: <strong>{order.customer_name}</strong></p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>???: {order.customer_phone}</p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>????: {order.district}, {order.upazila}</p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>??????: {order.address}</p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '6px 0 2px 0', fontWeight: 'bold' }}>?????? #: {order.id}</p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '2px 0' }}>?????: {new Date(order.created_at).toLocaleDateString('bn-BD')}</p>
            </div>
            <div style={{ background: PINK }} />
            <div style={{ padding: '14px', background: PINK_LIGHT }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                <img src={LOGO_URL} alt="????" style={{ height: '36px', width: 'auto', borderRadius: '6px' }} />
                <div>
                  <h1 style={{ fontSize: '16px', fontWeight: 'bold', color: PINK, margin: 0 }}>????? ?????</h1>
                </div>
              </div>
              ...
              <p style={{ fontSize: '11px', color: '#374151', margin: '6px 0 2px 0', fontWeight: 'bold' }}>?????: {new Date(order.created_at).toLocaleDateString('bn-BD')}</p>
              <p style={{ fontSize: '11px', color: '#374151', margin: '2px 0' }}>????: {new Date(order.created_at).toLocaleTimeString('bn-BD')}</p>
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 4px', marginBottom: '4px', borderBottom: '2px solid #374151' }}>
            <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#374151', margin: 0 }}>????</p>
            <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#374151', margin: 0 }}>????</p>
          </div>
        {(order.order_items || []).map((item: any, i: number) => {
            const displayName = item.product_name || item.products?.name || '????';
              ...
              <div key={i} style={{ borderBottom: '1px dashed #d1d5db', padding: '8px 4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ flex: 1, paddingRight: '8px' }}>
                    ...
                    {item.selected_size && <p style={{ fontSize: '11px', color: '#db2777', fontWeight: 'bold', margin: '0 0 2px 0' }}>????: {item.selected_size}</p>}
                    {displayCode && <p style={{ fontSize: '11px', color: '#3b82f6', margin: '0 0 2px 0' }}>???: {displayCode}</p>}
          ...
          <p style={{ textAlign: 'center', fontSize: '12px', color: '#9ca3af', marginTop: '16px', borderTop: '1px solid #e5e7eb', paddingTop: '10px' }}>??????? ????? ?????? ???????? ???? ????! ??</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <button onClick={onBack} style={{ background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '13px', cursor: 'pointer' }}>? ?????</button>
      ...
      <div style={{ padding: '12px 16px' }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="?? ???, ??? ?? ?????? ?????..." style={{ border: '2px solid #e5e7eb', borderRadius: '10px', padding: '8px 12px', width: '100%', fontSize: '13px', outline: 'none', marginBottom: '10px', boxSizing: 'border-box', color: '#1f2937' }} />
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
          <div style={{ background: '#dcfce7', borderRadius: '12px', padding: '12px', textAlign: 'center', border: '1px solid #bbf7d0' }}>
            <p style={{ fontSize: '12px', color: '#15803d', margin: '0 0 4px 0' }}>?? ???</p>
          ...
          <div style={{ background: '#dbeafe', borderRadius: '12px', padding: '12px', textAlign: 'center', border: '1px solid #bfdbfe' }}>
            <p style={{ fontSize: '12px', color: '#1d4ed8', margin: '0 0 4px 0' }}>?? ??????</p>
            <p style={{ fontSize: '20px', fontWeight: 'bold', color: '#1d4ed8', margin: 0 }}>{totalOrders} ??</p>
          </div>
        </div>
  ...
  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '16px' }}>
    <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '680px', maxHeight: '90vh', overflowY: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderBottom: '1px solid #e5e7eb' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: PINK, margin: 0 }}>?????? #{selected.order_id}</h2>
      ...
      <div style={{ padding: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2px 1fr', border: `2px solid ${PINK}`, borderRadius: '8px', overflow: 'hidden', marginBottom: '16px' }}>
          <div style={{ padding: '14px', background: PINK_LIGHT }}>
            ...
            <p style={{ fontSize: '11px', color: '#4b5563', margin: '2px 0' }}>?? sohelmart.com</p>
            <p style={{ fontSize: '11px', color: '#4b5563', margin: '2px 0' }}>?? 01872149655</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '6px 0 2px 0', fontWeight: 'bold' }}>?????: {new Date(selected.order?.created_at).toLocaleDateString('bn-BD')}</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '2px 0' }}>????: {new Date(selected.order?.created_at).toLocaleTimeString('bn-BD')}</p>
          </div>
          <div style={{ background: PINK }} />
          <div style={{ padding: '14px' }}>
            <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#1d4ed8', margin: '0 0 6px 0' }}>?? ???????? ????</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>???: <strong>{selected.order?.customer_name}</strong></p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>???: {selected.order?.customer_phone}</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>????: {selected.order?.district}, {selected.order?.upazila}</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '3px 0' }}>??????: {selected.order?.address}</p>
            <p style={{ fontSize: '11px', color: '#374151', margin: '6px 0 2px 0', fontWeight: 'bold' }}>?????? #: {selected.order_id}</p>
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 4px', marginBottom: '4px', borderBottom: '2px solid #374151' }}>
          <p style={{ fontSize: '13px', fontWeight: 'bold', color: '#374151', margin: 0 }}>????</p>
  ...
  return (
    <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
     {pageRequests.length > 0 && (
      ...
      {sellers.length === 0 && <p style={{ textAlign: 'center', color: '#9ca3af', padding: '32px 0' }}>???? ????? ???</p>}
      {sellers.map((seller) => (
        <div key={seller.id} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div onClick={() => setSelectedSeller(seller)} style={{ cursor: 'pointer', flex: 1 }}>
              ...
              <p style={{ fontSize: '11px', color: '#2563eb', margin: '4px 0 0 0' }}>?? ????? ???? ??????? ?????</p>
...
function WithdrawalManagement() {
  ...
  return (
    <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {requests.length === 0 && <p style={{ textAlign: 'center', color: '#9ca3af', padding: '32px 0' }}>???? ?????????? ???</p>}
      {requests.map((req) => (
        <div key={req.id} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#555', margin: '2px 0' }}>{req.method === 'bkash' ? '?? ?????' : req.method === 'nagad' ? '?? ???' : '?? ??????'} ? {req.account_number}</p>
    ...
    async function checkUrlProduct() {
      ...
      if (productId) {
        ...
        if (!isMoha) {
          ...
          // ?? Mohasagor ??????????? ????: mohasagor_assignments ???? page_id ??? ???, ??? page ?? branch ????? ??? ??? ???
      ...
      const channel = supabase.channel('orders').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, async (payload) => {
        ...
        if (role === 'admin') {
          ...
          if (fcmToken) {
            ...
            await fetch('https://jthdtmqrapnfmmmeuqsw.supabase.co/functions/v1/send-notification', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer sb_publishable_Eoh22VBAPMLBFnhyXMkq6Q_LqIbOw6J` }, body: JSON.stringify({ token: fcmToken, title: '?? ???? ??????!', body: `?????? #${order.id} - ${order.total_amount} Tk` }) });
          }
        }
      }).subscribe();
          ...
          <div style={{ background: 'white', padding: '32px', borderRadius: '21px', boxShadow: '0 4px 20px rgba(219,39,119,0.15)', position: 'relative', overflow: 'hidden', minHeight: '280px' }}>
            ...
            <div style={{ position: 'relative', zIndex: 20 }}>
              <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                <img src={LOGO_URL} alt="????" style={{ height: '80px', width: 'auto', borderRadius: '12px', cursor: 'pointer', display: 'block', margin: '0 auto 12px' }}
        ...
        {showLoginModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
            <div style={{ background: 'white', borderRadius: '20px', padding: '24px', maxWidth: '340px', width: '100%', margin: '0 16px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: PINK, textAlign: 'center', marginBottom: '16px' }}>?? ????</h2>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                ...
                <button onClick={() => setLoginType('editor')} style={{ padding: '10px', borderRadius: '10px', fontSize: '14px', fontWeight: '600', border: '2px solid', cursor: 'pointer', borderColor: loginType === 'editor' ? PINK : '#e5e7eb', background: loginType === 'editor' ? PINK : 'white', color: loginType === 'editor' ? 'white' : '#374151' }}>?? Editor</button>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} placeholder={loginType === 'admin' ? 'Admin ??????????' : 'Editor ??????????'} style={{ border: `2px solid ${PINK_BORDER}`, borderRadius: '8px', padding: '10px 12px', width: '100%', fontSize: '14px', marginBottom: '8px', boxSizing: 'border-box', outline: 'none', color: '#1f2937' }} autoFocus />
              {loginError && <p style={{ color: '#ef4444', fontSize: '12px', marginBottom: '8px' }}>{loginError}</p>}
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={handleLogin} style={{ background: PINK, color: 'white', border: 'none', borderRadius: '8px', padding: '10px', flex: 1, fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>????</button>
                <button onClick={() => { setShowLoginModal(false); setPassword(''); setLoginError(''); }} style={{ background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '8px', padding: '10px 16px', fontSize: '14px', cursor: 'pointer' }}>?????</button>
              </div>
            </div>
          </div>
        )}
      ...
      {showCustomerAuth && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, overflowY: 'auto', background: '#fdf2f8' }}>
          <div style={{ background: '#be185d', color: 'white', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', position: 'sticky', top: 0, zIndex: 100 }}>
            ...
            <h2 style={{ fontSize: '16px', fontWeight: 'bold', margin: 0 }}>???? / ????????????</h2>
          </div>
       ...
       <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex' }}>
          ...
          <div style={{ position: 'relative', marginLeft: 'auto', width: '100%', maxWidth: '380px', background: 'white', height: '100%', overflowY: 'auto', boxShadow: '-4px 0 20px rgba(0,0,0,0.15)' }}>
            <div style={{ background: PINK, color: 'white', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontWeight: 'bold', fontSize: '18px', margin: 0 }}>?? Settings</h2>
        ...
        <div style={{ padding: '16px' }}>
              ...
              {!showEmailForm && !showMohasagorAdmin && 'email send'.includes(settingsSearch.toLowerCase()) && (
                <button onClick={() => setShowEmailForm(true)} style={{ width: '100%', background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', fontSize: '15px', fontWeight: '600', color: '#1f2937', textAlign: 'left', marginBottom: '8px' }}>
                  <span style={{ fontSize: '20px' }}>??</span> Email Send
                          ...
                          {!showEmailForm && !showMohasagorAdmin && !showBranchControl && '??????? ????????? ????????'.includes(settingsSearch) && (
              ...
              {showMohasagorAdmin && (
                <div>
                  <button onClick={() => setShowMohasagorAdmin(false)} style={{ background: 'none', border: 'none', color: PINK, fontSize: '13px', cursor: 'pointer', marginBottom: '8px', padding: 0 }}>? ?????</button>
                  <MohasagorAdmin branchId={1} onAssign={() => setMohasagorRefresh(r => r + 1)} />
                </div>
              )}
              {showBranchControl && (
                <div>
                  <button onClick={() => setShowBranchControl(false)} style={{ background: 'none', border: 'none', color: PINK, fontSize: '13px', cursor: 'pointer', marginBottom: '8px', padding: 0 }}>? ?????</button>
                  <p style={{ fontSize: '13px', color: '#6b7280', margin: '0 0 12px 0' }}>???? ???? ???? ???? ???????? ?????? ???? ?????? ???? ?????</p>
              ...
              <div style={{ background: '#fdf2f8', border: `1px solid ${PINK_BORDER}`, borderRadius: '12px', padding: '16px' }}>
                <button onClick={() => setShowEmailForm(false)} style={{ background: 'none', border: 'none', color: PINK, fontSize: '13px', cursor: 'pointer', marginBottom: '8px', padding: 0 }}>? ?????</button>
                <p style={{ fontWeight: 'bold', fontSize: '15px', color: PINK, margin: '0 0 4px 0' }}>?? ??????????? ????? ?????</p>
                <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 12px 0' }}>?????? {'{name}'} ????? ?????????? ??? ??? ????</p>
                <input
                  value={emailSubject}
                ...
                <button
                  onClick={async () => {
                    if (!emailSubject || !emailMessage) { alert('???????? ??? ????? ?????!'); return; }
                    if (!confirm('?? ?????????? ????? ?????? ???? ????????')) return;
                    ...
                    try {
                      ...
                      if (data.ok) { alert(`? ${data.sent} ?? ?????????? ????? ?????? ??????!`); setEmailSubject(''); setEmailMessage(''); }
...
{sendingEmail ? '?????? ?????...' : '?? ????? ?????'}
      ...
      {showAdminDrawer && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex' }}>
          ...
          <div style={{ position: 'relative', marginLeft: 'auto', width: '100%', maxWidth: '380px', background: 'white', height: '100%', overflowY: 'auto', boxShadow: '-4px 0 20px rgba(0,0,0,0.15)' }}>
            <div style={{ background: PINK, color: 'white', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontWeight: 'bold', fontSize: '18px', margin: 0 }}>{role === 'editor' ? `?? ${localStorage.getItem('editor_page_name') || 'Editor'} Panel` : localStorage.getItem('current_page_id') ? `?? ${localStorage.getItem('current_page_name') || 'Admin'} Panel` : '?? Admin Panel'}</h2>
            ...
            <div style={{ padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div style={{ background: '#eff6ff', borderRadius: '12px', padding: '12px', textAlign: 'center', border: '1px solid #bfdbfe' }}><p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 4px 0' }}>?? Orders</p><p style={{ fontSize: '22px', fontWeight: 'bold', color: '#1d4ed8', margin: 0 }}>{filteredOrders2} ??</p></div>
              </div>
            </div>
            {role === 'admin' && (
              <div style={{ padding: '0 16px 12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f9fafb', borderRadius: '12px', padding: '12px' }}>
                  <div><p style={{ fontSize: '14px', fontWeight: '500', color: '#374151', margin: '0 0 2px 0' }}>??? Auto Print</p><p style={{ fontSize: '12px', color: '#9ca3af', margin: 0 }}>???? ??????? ??? ???????</p></div>
                  <button onClick={toggleAutoPrint} style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: '500', border: 'none', cursor: 'pointer', background: autoPrint ? PINK : '#e5e7eb', color: autoPrint ? 'white' : '#6b7280' }}>{autoPrint ? '? ????' : '? ????'}</button>
                </div>
              </div>
            )}
  ...
  <button key={t.key} onClick={() => { setAdminTab(t.key); if (t.key === 'orders') markAllRead(); }} style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap', border: 'none', cursor: 'pointer', background: adminTab === t.key ? PINK : '#f3f4f6', color: adminTab === t.key ? 'white' : '#374151', position: 'relative' }}>
    ...
    {t.key === 'orders' && unreadCount > 0 && (
      <span style={{ position: 'absolute', top: '-6px', right: '-6px', background: '#ef4444', color: 'white', fontSize: '10px', minWidth: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', padding: '0 3px' }}>{unreadCount}</span>
            ...
            {adminTab === 'orders' && (
              <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {dateFilteredOrders.length === 0 && <p style={{ textAlign: 'center', color: '#9ca3af', padding: '32px 0' }}>???? ?????? ???</p>}
                  ...
                  <div key={order.id} onDoubleClick={() => setSelectedOrder(order)} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '12px', cursor: 'pointer', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); setSelectedOrder(order); }} style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: '8px', padding: '4px 10px', fontSize: '12px', cursor: 'pointer' }}>??? Print/Save</button>
                      </div>
                    </div>
                    <div style={{ marginTop: '8px', borderTop: '1px solid #f3f4f6', paddingTop: '8px' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 'bold', color: PINK, marginTop: '4px' }}><span>???:</span><span>{order.total_amount} Tk</span></div>
                    </div>
                    <p style={{ fontSize: '11px', color: '#9ca3af', textAlign: 'center', marginTop: '4px' }}>???? ????? ???? ???? ????? ?????</p>
                  </div>
                ))}
              </div>
            )}
